import React from 'react'

const Allacess = () => {
  return (
   <section className="section no-padding">
  <div className="container-footer">
    <div className="outline-div margin-regular">
      <div className="cta-block-d right">
        <div
          id="w-node-_5032d3f4-3c0f-a63e-8383-7260a86e230c-b8a51723"
          className="cta-content-wrapper _2-grid"
        >
          <div className="wrap-v-large">
            <img
              src="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/6868cb51d74e445c7fd5a2c3_Group%2029.png"
              loading="lazy"
              sizes="100vw"
              height={50}
              alt=""
              srcSet="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/6868cb51d74e445c7fd5a2c3_Group%2029-p-500.png 500w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/6868cb51d74e445c7fd5a2c3_Group%2029.png 626w"
              className="h3-image"
              style={{
                maxWidth: "41%",
                height: "auto",
                width: "auto",
              }}
            />
            <div>
              <h3 className="white">The one ultimate plan,</h3>
              <div className="pricing-price-block">
                <h3 className="pricing white">
                  {" "}
                  for just <span className="change-price">$17</span>
                  <span className="discount">-30%</span>
                </h3>
                <div className="pricing-price-detail white">
                  <p className="paragraph-small-2 month">/month</p>
                </div>
              </div>
            </div>
            <a
              data-wf-native-id-path="5032d3f4-3c0f-a63e-8383-7260a86e2311"
              data-wf-ao-click-engagement-tracking="true"
              data-wf-element-id="5032d3f4-3c0f-a63e-8383-7260a86e2311"
              href="https://start.dropcourse.com/b/3cI9AU5SW9Gi2qu9Mi6oo04"
              data-wf-event-ids={157035618}
              className="button-primary white w-inline-block"
              rel="noopener"
            >
              <div>
                Get instant access <span className="arrow">→</span>
              </div>
            </a>
          </div>
          <div className="outline-div blur">
            <img
              src="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/688ca93db3df9eadeff75fb8_Frame%20179.avif"
              loading="lazy"
              sizes="100vw"
              srcSet="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/688ca93db3df9eadeff75fb8_Frame%20179-p-500.avif 500w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/688ca93db3df9eadeff75fb8_Frame%20179-p-800.avif 800w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/688ca93db3df9eadeff75fb8_Frame%20179-p-1080.avif 1080w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/688ca93db3df9eadeff75fb8_Frame%20179.avif 2634w"
              alt=""
              className="image-cover highlight"
            />
          </div>
        </div>
      </div>
    </div>
    <div className="outline-div _100">
      <div className="card white affiliate">
        <div className="wrap-v-regular align-left middle">
          <img
            src="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/68d5f6e06da9b951eef9f96e_Group%2019.png"
            loading="lazy"
            sizes="100vw"
            srcSet="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/68d5f6e06da9b951eef9f96e_Group%2019-p-500.png 500w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/68d5f6e06da9b951eef9f96e_Group%2019-p-800.png 800w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/68d5f6e06da9b951eef9f96e_Group%2019-p-1080.png 1080w, https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/68d5f6e06da9b951eef9f96e_Group%2019.png 1426w"
            alt=""
            className="h3-image"
            style={{
              maxWidth: "41%",
              height: "auto",
              width: "auto",
            }}
          />
        </div>
        <p>
          Get paid 50% lifetime commissions when a<em>nyone </em>you refer
          becomes a paid member. Only available to paid members.
        </p>
        <div
          id="w-node-_3942ba87-d10e-fd8d-49c0-96654ad65c3d-b8a51723"
          className="message"
        >
          <img
            src="https://cdn.prod.website-files.com/67b96fd14bb10523b8a51725/6843c6f9085a449fceec77b0_payments_24dp_00420B_FILL0_wght400_GRAD0_opsz24.svg"
            loading="lazy"
            width={17}
            alt=""
            className="_75"
          />
          <p className="_75">Get paid 50% commission for life</p>
        </div>
      </div>
    </div>
  </div>
</section>

  )
}

export default Allacess